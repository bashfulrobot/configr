...
func NewOptimizedLoader(logger *log.Logger, cache *pkg.CacheManager) *OptimizedLoader {
...
func (ol *OptimizedLoader) LoadConfigurationOptimized(configPath string) (*Config, []string, error) {
	startTime := time.Now()
	...
	if err := ol.cache.SaveCachedConfig(config, configPaths); err != nil {
...
func (ol *OptimizedLoader) extractIncludePaths(configPath string) ([]string, error) {
	// Create a temporary viper instance to read just the includes
	tempViper := viper.New()
	...
	includes := tempViper.GetStringSlice("includes")
	return includes, nil
}
...
func (ol *OptimizedLoader) collectIncludePaths(includes []string, baseDir string, allPaths *[]string, visited map[string]bool) error {
	for _, include := range includes {
		...
		} else if !filepath.HasSuffix(includePath, ".yaml") && !filepath.HasSuffix(includePath, ".yml") {
...
func (ol *OptimizedLoader) GetCacheStats() (*pkg.CacheStats, error) {
